(ns redknee.main
  "Kotodama WASM entrypoint for the Redknee clean-room actor (L4) — Clojure port.
  ...
  facts (`redknee.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "redknee")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"redknee.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "redknee-compat" :tier tier :entities entities} 200])
...
(defn start! [] :redknee-compat/ready)
